#!/usr/bin/env bash
#
# Launch all 1b full_wsd lact jobs.
...
#
# Usage:
#   bash scripts/launch_lact_full_wsd.sh        # launch all
#   bash scripts/launch_lact_full_wsd.sh --dry   # print commands only
#
...
run_cmd uv run theseus submit lact_full_wsd-p1 \
    ...
    -s configs/continual/cg_cfq/1b/lact_full_wsd.yaml \
...
run_cmd uv run theseus submit lact_full_wsd-p1 \
    ...
    -s configs/continual/cg_grammar/1b/lact_full_wsd.yaml \
...
run_cmd uv run theseus submit lact_full_wsd-p1 \
    ...
    -s configs/continual/cg_safety/1b/lact_full_wsd.yaml \
...
run_cmd uv run theseus submit lact_full_wsd-p1 \
    ...
    -s configs/continual/ds_nlu/1b/lact_full_wsd.yaml \
...
echo "=== All 7 lact jobs submitted ==="
